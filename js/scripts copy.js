$(document).ready(function() {
  ...
  loadCSVFile('songs.csv', function(data) {
    ...
    // console.log(data);
    // You can process and use this data to populate your song list
    songs = data;
    console.log(songs);
  ...
  loadCSVFile('setlists.csv', function(data) {
    // 'data' now contains the parsed CSV data for setlists
    // console.log(data);
  ...
  function populateSetlist(setlistArray) {
    ...
    console.log('Setlist array:', setlistArray);
    console.log('Setlist song list items:', setlistSongList.html()); // Log the HTML content of the setlist song list
  }
  ...
  function initSetlistControls(){
    ...
    console.log("Number of Songs in Playlist: " + String(setlistArray.length).padStart(2, '0'));
  ...
  $('#next-song-button').on('click', function() {
    ...
    //nextSetlistSongPath = $('#setlist-song-list li').eq(nextSetlistSongnumber).attr('data-path');
    //loadSongHtml(nextSetlistSongPath);
    //previousSetlistSongnumber = currentSetlistSongnumber + 1
